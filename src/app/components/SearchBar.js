...
export default function SearchBar() {
    ...
    const rechercher = async () => {
        try {
            ...
            router.push(`/mangaSearched?manga=${encodeURIComponent(searchValue)}`);
    ...
    <div
        id="SearchBar"
        className="relative inline-flex items-center text-gray-600 focus-within:text-gray-400 w-1/3 min-w-[300px]"
    >
        ...
        <input
            ...
            type="search"
            name="q"
            className="py-2 text-sm text-gray-600 bg-gray-200 rounded-md pl-10 focus:outline-none focus:bg-gray-200 focus:text-gray-600 w-full"
            placeholder="Search..."
            autoComplete="off"
        />
        <button className={styles.btnSearch} onClick={rechercher}>
            Search
        </button>
    </div>
</div>
